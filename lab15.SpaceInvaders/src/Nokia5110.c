...
Nokia5110_Init(void) 
{
    ...
    lcdwrite(COMMAND, CONTRAST); // try 0xB1 (for 3.3V red SparkFun), 0xB8
                                 // (for 3.3V blue SparkFun), 0xBF if your display
                                 // is too dark, or 0x80 to 0xFF if experimenting
...
void
Nokia5110_OutChar(unsigned char data) 
{
    int i;
...
// outputs: none
void
Nokia5110_SetCursor(unsigned char newX, unsigned char newY) 
{
...
// assumes: LCD is in default horizontal addressing mode (V = 0)
void
Nokia5110_DrawFullImage(const char *ptr) 
{
    int i;
    Nokia5110_SetCursor(0, 0);
    for (i = 0; i < (MAX_X * MAX_Y / 8); i = i + 1) {
...
// outputs: none
void
Nokia5110_PrintBMP(unsigned char xpos, unsigned char ypos,
...
void
Nokia5110_SetPixel(unsigned char x, unsigned char y) 
{
...
// Pixel level functions by Tomas
void
Nokia5110_ClearPixel(unsigned char x, unsigned char y) 
{
    unsigned short PixelByte;   // byte# in screen buffer
    unsigned char PixelBit;     // bit# in byte
    if ((x < 84) && (y < 48)) { // check screen boundaries
