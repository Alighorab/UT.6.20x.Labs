...
// I.e., if you wish to use code from sprite.c or sound.c, move that code in
// this file 4) It must compile with the 32k limit of the free Keil
...
// includes two blacked out columns on the left and right sides of the image to
// prevent smearing when moved 2 pixels to the left or right width=18 x height=8
...
// small, fast bonus enemy that occasionally speeds across the top of the screen
// after enough enemies have been killed to make room for it includes two
// blacked out columns on the left and right sides of the image to prevent
// smearing when moved 2 pixels to the left or right width=20 x height=7
...
// small shield floating in space to cover the player's ship from enemy fire
// (undamaged) width=18 x height=5
...
// small shield floating in space to cover the player's ship from enemy fire
// (moderate generic damage) width=18 x height=5
...
// small shield floating in space to cover the player's ship from enemy fire
// (heavy generic damage) width=18 x height=5
...
// large explosion that can be used upon the demise of the player's ship (first
// frame) width=18 x height=8
...
// large explosion that can be used upon the demise of the player's ship (second
// frame) width=18 x height=8
...
// a missile in flight
// includes one blacked out row on the top, bottom, and right of the image to
// prevent smearing when moved 1 pixel down, up, or left width=4 x height=9
...
// a missile in flight
// includes one blacked out row on the top, bottom, and left of the image to
// prevent smearing when moved 1 pixel down, up, or right width=4 x height=9
...
// includes one blacked out row on the top and bottom of the image to prevent
// smearing when moved 1 pixel up or down width=2 x height=9
...
void
Timer2_Init(unsigned long period) 
{
...
void
Delay100ms(unsigned long count) 
{
